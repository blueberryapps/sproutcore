...
/** @scope SC.Record.prototype */ {
  ...
  */
  storeDidChangeProperties: function(statusOnly, keys) {
    ...
    if (statusOnly) this.notifyPropertyChange('status');
      ...
      // if rec is a parentRecord
      if (this.isParentRecord){
        // debugger;
        SC.Logger.log('What is going on with: %@'.fmt(keys));
      }
    }
  },
  ...
  */
   ...
   */
  registerChildRecord: function(value, key) {
    ...
    if (value && value.get && value.get('isRecord')) {
      ...
      recordType = this._materializeChildRecordType(value, key);
      childRecord = this.createChildRecord(recordType, value);
    }
    if (childRecord){
  ...
  _materializeChildRecordType: function(value, key){
  ...
  /**
    ...
    @param {SC.Record} childRecordType The type of the child record to create.
  ...
  createChildRecord: function(childRecordType, hash) {
    ...
    SC.run(function() {
      ...
      if (!id && (pk = childRecordType.prototype.primaryKey)) {
        id = hash[pk];
        // In case there isnt a primary key supplied then we create on
        // on the fly
        sk = id ? store.storeKeyExists(childRecordType, id) : null;
        if (sk){
  ...
  /**
    finds a child record instance.

    @param {SC.Record} childRecordType The type of the child record to create.
    @param {Hash} hash The hash of attributes to apply to the child record.
    (may be null)
   */
  findChildRecord: function(childRecordType, id){
    SC.Logger.log('SC.Record#findChildRecord()');
  },
